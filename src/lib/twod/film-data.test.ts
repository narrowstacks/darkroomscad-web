...
describe("film-data", () => {
  it("matches the SCAD film table for key formats", () => {
    expect(FILM_FORMATS["35mm"]).toEqual({ height: 36, width: 24, pegDistance: 37.5, typeName: "35MM" });
    expect(FILM_FORMATS["6x6"]).toEqual({ height: 56, width: 56, pegDistance: 64, typeName: "6x6" });
    expect(FILM_FORMATS["4x5"]).toEqual({ height: 120, width: 95, pegDistance: 104.1, typeName: "4X5" });
    expect(FILM_FORMATS["6x9 filed"]).toEqual({ height: 86, width: 58, pegDistance: 64, typeName: "F6x9" });
    expect(FILM_FORMATS["half frame filed"]).toEqual({ height: 21, width: 28, pegDistance: 37.5, typeName: "FILEDHALF" });
  });
